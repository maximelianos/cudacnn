...
void byteToFloat(imgData img_byte, imgDataF img_float, int byte_size) {
	for (int i = 0; i < byte_size; ++i) {
		img_float.pixels[i] = img_byte.pixels[i];
	}
}

void floatToByte(imgDataF img_float, imgData img_byte, int byte_size) {
	for (int i = 0; i < byte_size; ++i) {
		img_byte.pixels[i] = img_float.pixels[i];
	}
}
